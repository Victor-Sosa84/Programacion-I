...
type
  TfrmPrincipal = class(TForm)
    ...
    Palabramsrepetida1: TMenuItem;
    EncontraryReemplazar1: TMenuItem;
    Matriz1: TMenuItem;
    Negativos1: TMenuItem;
    ...
    procedure Palabramsrepetida1Click(Sender: TObject);
    procedure EncontraryReemplazar1Click(Sender: TObject);
    procedure Matriz1Click(Sender: TObject);
    procedure Negativos1Click(Sender: TObject);
  private
...
procedure TfrmPrincipal.EncontraryReemplazar1Click(Sender: TObject);
begin
   MiArchivo.findandreplace(InputBox('Buscar','Reemplazar por: ',''),InputBox('Buscar','Reemplazar: ',''),Memo1.Lines);
end;
...
begin
    MiArchivo.matriz(Memo1.Lines);
end;

procedure TfrmPrincipal.Negativos1Click(Sender: TObject);
var
  i: integer;
  lineas: TStrings;
  r: string;
begin
  lineas:= MiArchivo.negativos(Memo1.Lines);

  r:='';
  for i := 0 to Lineas.Count-1 do
     r:=r+lineas[i]+sLineBreak;

  ShowMessage(r);
end;

procedure TfrmPrincipal.Palabramsrepetida1Click(Sender: TObject);
begin
    ShowMessage(MiArchivo.PalMasRepetida(Memo1.Lines));
end;
